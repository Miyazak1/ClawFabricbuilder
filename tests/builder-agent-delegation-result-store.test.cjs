...
function temporaryDatabase(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'clawfabric-builder-agent-delegation-results-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  return path.join(root, 'agent-delegation-results.sqlite');
}
...
function recordRequest(delegationRecord, result) {
  return { delegation: delegationRecord, result };
}
...
function assertStoreError(fn, code = null) {
  assert.throws(
    fn,
    (error) => {
      assert.ok(error instanceof BuilderAgentDelegationResultStoreError);
...
test('records delegation result receipts then restores them after restart', (t) => {
  const databasePath = temporaryDatabase(t);
  const store = createBuilderAgentDelegationResultStore(databasePath);
  const { delegationRecord } = fixture();
  const result = resultRecord(delegationRecord);

  assert.equal(store.store_version, BUILDER_AGENT_DELEGATION_RESULT_STORE_VERSION);
  const recorded = store.record_result(recordRequest(delegationRecord, result));
  assert.equal(recorded.result_version, BUILDER_AGENT_DELEGATION_RESULT_STORE_RESULT_VERSION);
  assert.equal(recorded.operation, 'delegation_result_recorded');
  assert.deepEqual(recorded.delegation_result.result, result);
  assert.deepEqual(recorded.delegation_result.delegation, delegationRecord);
  assert.equal(recorded.delegation_result_evidence.delegation_result_authority, 'main_owned_agent_delegation_result_store');
  assert.equal(recorded.delegation_result_evidence.renderer_authority, 'not_present');
  assert.equal(recorded.delegation_result_evidence.ipc_authority, 'not_present');
  assert.equal(recorded.delegation_result_evidence.child_assignment_authority, false);
  assert.equal(recorded.delegation_result_evidence.model_dispatch, false);
  assert.equal(recorded.delegation_result_evidence.tool_dispatch, false);
  assert.equal(recorded.delegation_result_evidence.permission_grant_authority, false);
  assert.equal(recorded.delegation_result_evidence.credential_storage, 'not_present');
  assert.equal(recorded.delegation_result_evidence.source_read, 'not_present');
  assert.equal(recorded.delegation_result_evidence.source_write, 'not_present');
  assert.equal(recorded.delegation_result_evidence.process_run, false);
  assert.equal(recorded.delegation_result_evidence.network_access, false);
  assert.equal(recorded.delegation_result_evidence.revision_authority, false);
  assert.equal(recorded.delegation_result_evidence.review_authority, false);
  assert.equal(recorded.delegation_result_evidence.artifact_authority, false);
  assert.equal(recorded.delegation_result_evidence.schema_version, BUILDER_AGENT_DELEGATION_RESULT_STORE_SCHEMA_VERSION);
  assert.equal(recorded.delegation_result_evidence.user_version, BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION);
  assert.match(recorded.delegation_result_evidence.schema_fingerprint_digest, /^[a-f0-9]{64}$/u);

  assert.equal(store.record_result(recordRequest(delegationRecord, result)).operation, 'delegation_result_replayed');

  const read = store.read_result({ delegation_result_id: result.delegation_result_id, owner_id: OWNER_ID });
  assert.equal(read.result_version, BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION);
  assert.equal(read.status, 'ready');
  assert.deepEqual(read.delegation_result.result, result);
  assert.equal(Object.isFrozen(read), true);
  assert.equal(Object.isFrozen(read.delegation_result), true);

  const parentList = store.list_parent_task_results({
    owner_id: OWNER_ID,
    project_id: PROJECT_ID,
    parent_task_id: TASK_ID,
  });
  assert.equal(parentList.status, 'ready');
  assert.equal(parentList.delegation_results.length, 1);
  assert.deepEqual(parentList.delegation_results[0].result, result);

  const childList = store.list_child_task_results({
    owner_id: OWNER_ID,
    project_id: PROJECT_ID,
    child_task_id: CHILD_TASK_ID,
  });
  assert.equal(childList.status, 'ready');
  assert.equal(childList.delegation_results.length, 1);
  assert.deepEqual(childList.delegation_results[0].result, result);
  store.close();

  const restarted = createBuilderAgentDelegationResultStore(databasePath);
  const restored = restarted.read_result({ delegation_result_id: result.delegation_result_id, owner_id: OWNER_ID });
  assert.equal(restored.status, 'ready');
  assert.deepEqual(restored.delegation_result.result, result);
  assert.deepEqual(restored.delegation_result.delegation, delegationRecord);
  restarted.close();
});

test('records multiple parent results while enforcing owner scope and one result per delegation', (t) => {
  const databasePath = temporaryDatabase(t);
  const store = createBuilderAgentDelegationResultStore(databasePath);
  const first = fixture();
  const firstResult = resultRecord(first.delegationRecord);
  store.record_result(recordRequest(first.delegationRecord, firstResult));
  ...
  assert.equal(
    store.record_result(recordRequest(second.delegationRecord, secondResult)).operation,
  ...
  assert.equal(parentList.delegation_results.length, 2);
  assert.deepEqual(parentList.delegation_results.map((entry) => entry.result.result.status), ['proposed', 'blocked']);
  assert.equal(
    store.read_result({ delegation_result_id: firstResult.delegation_result_id, owner_id: OTHER_OWNER_ID }).status,
    'absent',
  );
  assert.equal(
    store.list_parent_task_results({ owner_id: OTHER_OWNER_ID, project_id: PROJECT_ID, parent_task_id: TASK_ID }).status,
    'absent',
  );

  const conflicting = resultRecord(first.delegationRecord, {
    observed_at_ms: 106,
    result: {
      status: 'failed',
  ...
  assertStoreError(
    () => store.record_result(recordRequest(first.delegationRecord, conflicting)),
    'builder_agent_delegation_result_store_conflict',
  );
  store.close();
});
...
test('rejects hostile input, malformed reads, and tampered rows', (t) => {
  ...
  assertStoreError(() => store.record_result({ ...recordRequest(delegationRecord, result), extra: true }));
  assertStoreError(() => store.read_result({
    delegation_result_id: result.delegation_result_id,
    owner_id: OWNER_ID,
    extra: true,
  }));
  assertStoreError(() => store.list_parent_task_results({
    owner_id: OWNER_ID,
    project_id: PROJECT_ID,
    parent_task_id: TASK_ID,
    extra: true,
  }));
  assertStoreError(() => store.list_child_task_results({
    owner_id: OWNER_ID,
    project_id: PROJECT_ID,
    child_task_id: CHILD_TASK_ID,
    extra: true,
  ...
  let getterCalls = 0;
  const accessor = recordRequest(delegationRecord, result);
  ...
  assertStoreError(() => store.record_result(new Proxy(recordRequest(delegationRecord, result), {
  ...
  store.record_result(recordRequest(delegationRecord, result));
  store.close();

  const raw = new DatabaseSync(databasePath);
  raw.prepare('UPDATE agent_delegation_results SET result_status = ? WHERE delegation_result_id = ?')
    .run('failed', result.delegation_result_id);
  raw.close();

  const corrupted = createBuilderAgentDelegationResultStore(databasePath);
  assertStoreError(
    () => corrupted.read_result({ delegation_result_id: result.delegation_result_id, owner_id: OWNER_ID }),
...
test('rejects schema drift and unsafe database paths', (t) => {
  ...
  const raw = new DatabaseSync(databasePath);
  raw.exec('CREATE TABLE unexpected_delegation_result_fact (id TEXT PRIMARY KEY) STRICT');
  raw.close();

  assertStoreError(
    () => createBuilderAgentDelegationResultStore(databasePath),
    'builder_agent_delegation_result_store_integrity_failed',
  );
  assertStoreError(
    () => createBuilderAgentDelegationResultStore(path.join('relative', 'agent-delegation-results.sqlite')),
  ...
  assertStoreError(
    () => createBuilderAgentDelegationResultStore(notDatabasePath),
    'builder_agent_delegation_result_store_unavailable',
  );
});

test('source boundary remains a main-only Agent delegation result store without runtime authority', () => {
  const source = fs.readFileSync(
    path.join(__dirname, '..', 'electron', 'builder-agent-delegation-result-store.cjs'),
    'utf8',
  );
  assert.match(source, /main_owned_agent_delegation_result_store/u);
  assert.match(source, /record_result/u);
  assert.match(source, /read_result/u);
  assert.match(source, /list_parent_task_results/u);
  assert.match(source, /list_child_task_results/u);
