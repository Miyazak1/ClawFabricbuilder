...
test('source remains a thin wrapper over the packaged canary authority', () => {
  ...
  assert.doesNotMatch(
    source,
    /safeStorage|ipcMain|ipcRenderer|BrowserWindow|fetch\s*\(|Authorization|Bearer|providerSettings\.replaceCurrent|codeGenerator\.(?:generate|retry|answer|rejectDraft)/iu,
  );
});
