...
test('submits one composer turn through main-owned work or explanation routing', async () => {
  ...
  const explicitBriefAnswer = await service.submit(request({
    instruction: '保存这个方向，后面按这个来：目标用户是小团队',
    existingProjectId: PROJECT_ID,
  }));
  ...
  assert.equal(explicitBriefAnswer.result_kind, 'explanation');
  assert.equal(explicitBriefAnswer.admissions.draft, 'not_created');
  ...
  assert.equal(lifecycle.calls.question.length, 13);
  assert.equal(lifecycle.calls.candidate.length, 3);
  assert.equal(lifecycle.calls.explanation.length, 13);
  assert.deepEqual(
  ...
  assert.deepEqual(startedEvents.map((event) => event.request_id), [
    ...
    explicitBriefAnswer.request_id,
  ...
  assert.deepEqual(startedEvents.map((event) => event.project_id), [
    ...
    explicitBriefAnswer.project_id,
...
test('keeps natural-language plan requests out of the submit build fallback', async () => {
  ...
  const service = createBuilderGenerationMainService({
    ...
    transport: async () => ({
      ...
      generated_text: JSON.stringify(providerExplanation({
        title: 'Plan request',
        summary: 'We should plan before changing files.',
        explanation: 'I can help plan first. No files were changed.',
      })),
    }),
  });
  ...
  assert.deepEqual(lifecycle.calls.question[0].route_decision_hint.matched_signals, ['explicit_plan']);
  assert.deepEqual(lifecycle.calls.question[0].route_decision_hint.required_permissions, []);
  assert.equal(lifecycle.calls.question[0].route_decision_hint.permission_result, 'not_required');
...
test('keeps convert-to-plan requests out of the submit build fallback', async () => {
  const sourceTree = createBuilderProjectSourceTree({
    files: [{ path: 'src/app.js', content: 'export const saved = true;\n' }],
  });
  const lifecycle = conversationService();
  ...
  const service = createBuilderGenerationMainService({
    ...
    transport: async () => ({
      ...
      generated_text: JSON.stringify(providerExplanation({
        title: 'Plan request',
        summary: 'We should create a plan before changing files.',
        explanation: 'I can help turn this into a plan. No files were changed.',
      })),
    }),
  });
