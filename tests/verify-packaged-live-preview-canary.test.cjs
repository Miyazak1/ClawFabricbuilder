...
test('live preview canary script is independent from release and verifies browser evidence', () => {
  ...
  assert.match(source, /preview_document_title_observed/u);
  ...
  assert.match(source, /reload_canvas_nonblank:\s*true/u);
  assert.match(source, /reload_javascript_executed:\s*true/u);
  assert.match(source, /reload_loopback_webcontents_observed:\s*true/u);
