...
function bridgeEvidence(
  ...
  return {
    bridge_contract: {
      bridge_version: 'builder-preload.v12',
...
function installBridge(page) {
  globalThis.clawfabricBuilder = {
    bridgeVersion: 'builder-preload.v12',
    codeGenerator: {
...
test('observes an unsaved draft before saving Version 1 through the real UI', async (t) => {
  ...
  assert.equal(evidence.bridge_contract.bridge_version, 'builder-preload.v12');
  ...
  assert.doesNotMatch(source, /replaceCurrent|codeGenerator\.(?:submit|generate|generateApprovedPlan|proposePlan|retry|answer|rejectDraft|steer)|projectWorkspace\.saveDraft|cancel/u);
...
test('retries a failed draft through visible UI without saving or leaking write authority', async (t) => {
  ...
  assert.doesNotMatch(
    evaluateEvents[0][1],
    /codeGenerator\.(?:submit|generate|generateApprovedPlan|proposePlan|retry|answer|rejectDraft|steer)|projectWorkspace\.saveDraft|providerSettings\.replaceCurrent/u,
  );
});
...
test('script source keeps credential out of argv/env/output and cannot enter ASAR authority', () => {
  ...
  assert.doesNotMatch(source, /permissions\.evaluate|providerSettings\.replaceCurrent|codeGenerator\.(?:submit|generate|generateApprovedPlan|proposePlan|retry|answer|rejectDraft|steer)|projectWorkspace\.(?:saveDraft|createLocalProject)/u);
  ...
  assert.match(preloadSource, /bridgeVersion:\s*['"]builder-preload\.v12['"]/u);
