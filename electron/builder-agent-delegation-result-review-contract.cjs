...
const BUILDER_AGENT_DELEGATION_RESULT_REVIEW_CONTRACT_VERSION =
  'builder-agent-delegation-result-review-contract.v1';
const BUILDER_AGENT_DELEGATION_RESULT_REVIEW_RECORD_VERSION =
  'builder-agent-delegation-result-review-record.v1';
const BUILDER_AGENT_DELEGATION_RESULT_REVIEW_RECORD_KIND =
  'builder_agent_delegation_result_review_record';
...
const DECISION_DISPLAY_SUMMARIES = Object.freeze({
  delegated_child_result_approved_for_parent_materialization:
    'Delegated result is approved for the parent materialization gate.',
  delegated_child_result_rejected_by_owner:
    'Delegated result was rejected by the owner.',
  delegated_child_result_acknowledged_without_materialization:
    'Delegated result was acknowledged without materialization.',
});
...
const AUTHORITY = Object.freeze({
  record_authority: 'main_agent_delegation_result_review_contract_v1',
...
const ERROR_MESSAGES = Object.freeze({
  builder_agent_delegation_result_review_contract_invalid:
    'Builder agent delegation result review could not be verified.',
});

class BuilderAgentDelegationResultReviewContractError extends Error {
  constructor() {
    super(ERROR_MESSAGES.builder_agent_delegation_result_review_contract_invalid);
    this.name = 'BuilderAgentDelegationResultReviewContractError';
    this.code = 'builder_agent_delegation_result_review_contract_invalid';
    this.stack = `${this.name}: ${this.message}`;
  }
}

function fail() {
  throw new BuilderAgentDelegationResultReviewContractError();
}
...
function safeAdmissionReference(admissionRecord, resultRecord, delegationRecord) {
  try {
    return sanitizeBuilderAgentDelegationResultAdmissionRecord(
      admissionRecord,
      resultRecord,
      delegationRecord,
    );
  } catch (error) {
    if (error instanceof BuilderAgentDelegationResultAdmissionContractError) fail();
    fail();
  }
}

function safeResult(value, admissionRecord) {
  ...
  if (
    status !== admissionRecord.result.status
    || summaryCode !== admissionRecord.result.summary_code
    || displaySummary !== admissionRecord.result.display_summary
  ) fail();
  return freezeDeep({
    status,
...
function safeReviewFields(value, admissionRecord, resultRecord, delegationRecord) {
  exactObject(value, INPUT_KEYS);
  const admission = safeAdmissionReference(admissionRecord, resultRecord, delegationRecord);
  ...
  const result = safeResult(valueAt(value, 'result'), admission);
  ...
  if (
    recordVersion !== BUILDER_AGENT_DELEGATION_RESULT_REVIEW_RECORD_VERSION
    || recordKind !== BUILDER_AGENT_DELEGATION_RESULT_REVIEW_RECORD_KIND
    || delegationResultAdmissionId !== admission.delegation_result_admission_id
    || delegationResultId !== admission.delegation_result_id
    || delegationId !== admission.delegation_id
    || parentAssignmentId !== admission.parent_assignment_id
    || parentAssignmentStatusId !== admission.parent_assignment_status_id
    || parentLeaseId !== admission.parent_lease_id
    || fromAgentId !== admission.from_agent_id
    || fromAgentVersionId !== admission.from_agent_version_id
    || toAgentId !== admission.to_agent_id
    || toAgentVersionId !== admission.to_agent_version_id
    || ownerId !== admission.owner_id
    || projectId !== admission.project_id
    || parentConversationId !== admission.parent_conversation_id
    || parentTaskId !== admission.parent_task_id
    || parentRunId !== admission.parent_run_id
    || childConversationId !== admission.child_conversation_id
    || childTaskId !== admission.child_task_id
    || childRunId !== admission.child_run_id
    || leaseHolderId !== admission.lease_holder_id
    || reviewedBy !== ownerId
    || reviewedAtMs < admission.admitted_at_ms
    ...
    || valueAt(value, 'review_contract') !== 'owner_review_recorded_before_parent_materialization'
...
function delegationResultReviewIdFor(admissionDigest, fields) {
  return `builder-agent-delegation-result-review:${sha256Canonical({
    agent_delegation_result_review_identity:
...
function createBuilderAgentDelegationResultReviewRecord(value, admissionRecord, resultRecord, delegationRecord) {
  try {
    const admission = safeAdmissionReference(admissionRecord, resultRecord, delegationRecord);
    const fields = safeReviewFields(value, admissionRecord, resultRecord, delegationRecord);
    const admissionDigest = sha256Canonical(admission);
    return freezeDeep({
      delegation_result_review_id: delegationResultReviewIdFor(admissionDigest, fields),
      delegation_result_admission_digest: admissionDigest,
      delegation_result_digest: admission.delegation_result_digest,
      delegation_definition_digest: admission.delegation_definition_digest,
      target_definition_digest: admission.target_definition_digest,
      ...fields,
      lifecycle: freezeDeep({ ...LIFECYCLE }),
      authority: freezeDeep({ ...AUTHORITY }),
    });
  } catch (error) {
    if (error instanceof BuilderAgentDelegationResultReviewContractError) throw error;
    fail();
  }
}

function sanitizeBuilderAgentDelegationResultReviewRecord(value, admissionRecord, resultRecord, delegationRecord) {
  try {
    const admission = safeAdmissionReference(admissionRecord, resultRecord, delegationRecord);
    ...
    if (
      admissionDigest !== sha256Canonical(admission)
      || delegationResultDigest !== admission.delegation_result_digest
      || delegationDefinitionDigest !== admission.delegation_definition_digest
      || targetDefinitionDigest !== admission.target_definition_digest
    ) fail();
    ...
    if (reviewId !== delegationResultReviewIdFor(admissionDigest, fields)) fail();
    return freezeDeep({
...
module.exports = Object.freeze({
  BUILDER_AGENT_DELEGATION_RESULT_REVIEW_CONTRACT_VERSION,
