...
const BUILDER_AGENT_DELEGATION_RESULT_STORE_VERSION = 'builder-agent-delegation-result-store.v1';
const BUILDER_AGENT_DELEGATION_RESULT_STORE_RESULT_VERSION = 'builder-agent-delegation-result-store-result.v1';
const BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION = 'builder-agent-delegation-result-store-read-result.v1';
const BUILDER_AGENT_DELEGATION_RESULT_STORE_SCHEMA_VERSION = 'builder-agent-delegation-result-store-schema.v1';
const BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION = 1;
const DATABASE_ID = 'builder-agent-delegation-result-store.v1';
...
const RECORD_RESULT_KEYS = Object.freeze(['delegation', 'result']);
const READ_RESULT_KEYS = Object.freeze(['delegation_result_id', 'owner_id']);
const LIST_PARENT_TASK_RESULTS_KEYS = Object.freeze(['owner_id', 'project_id', 'parent_task_id']);
const LIST_CHILD_TASK_RESULTS_KEYS = Object.freeze(['owner_id', 'project_id', 'child_task_id']);
const MAX_TASK_RESULTS = 128;
const MAX_RECEIPT_JSON_BYTES = 64 * 1024;
const CREATE_SCHEMA_SQL = Object.freeze([
  `CREATE TABLE agent_delegation_results (
    delegation_result_id TEXT NOT NULL PRIMARY KEY,
    ...
    CHECK (schema_version = 'builder-agent-delegation-result-store-schema.v1'),
    CHECK (record_version = 'builder-agent-delegation-result-record.v1'),
    CHECK (observed_at_ms >= 0),
    CHECK (result_status IN ('proposed', 'blocked', 'failed')),
    CHECK (summary_code IN (
    ...
    CHECK (return_contract = 'child_result_returned_for_parent_review'),
  ...
  'CREATE INDEX agent_delegation_results_parent_task_idx ON agent_delegation_results(owner_id, project_id, parent_task_id, observed_at_ms, delegation_result_id)',
  'CREATE INDEX agent_delegation_results_child_task_idx ON agent_delegation_results(owner_id, project_id, child_task_id, observed_at_ms, delegation_result_id)',
  'CREATE INDEX agent_delegation_results_delegation_idx ON agent_delegation_results(owner_id, delegation_id)',
  'CREATE INDEX agent_delegation_results_target_agent_idx ON agent_delegation_results(owner_id, to_agent_id, observed_at_ms, delegation_result_id)',
]);
const ERROR_MESSAGES = Object.freeze({
  builder_agent_delegation_result_store_invalid: 'Builder agent delegation result could not be verified.',
  builder_agent_delegation_result_store_not_found: 'Builder agent delegation result is unavailable.',
  builder_agent_delegation_result_store_conflict: 'Builder agent delegation result changed before it could be recorded.',
  builder_agent_delegation_result_store_integrity_failed: 'Builder agent delegation result integrity could not be verified.',
  builder_agent_delegation_result_store_resource_exceeded: 'Builder agent delegation result limits were reached.',
  builder_agent_delegation_result_store_unavailable: 'Builder agent delegation result storage is unavailable.',
});

class BuilderAgentDelegationResultStoreError extends Error {
  constructor(code = 'builder_agent_delegation_result_store_invalid') {
    const selected = Object.hasOwn(ERROR_MESSAGES, code)
      ? code
      : 'builder_agent_delegation_result_store_invalid';
    super(ERROR_MESSAGES[selected]);
    this.name = 'BuilderAgentDelegationResultStoreError';
    this.code = selected;
...
function fail(code) {
  throw new BuilderAgentDelegationResultStoreError(code);
}
...
function exactObject(value, keys) {
  if (!isPlainObject(value)) fail('builder_agent_delegation_result_store_invalid');
  const actual = Reflect.ownKeys(value);
  if (
    actual.length !== keys.length
    || actual.some((key) => typeof key !== 'string' || !keys.includes(key))
  ) fail('builder_agent_delegation_result_store_invalid');
...
function safeTimestamp(value) {
  if (!Number.isSafeInteger(value) || value < 0) fail('builder_agent_delegation_result_store_invalid');
  return value;
}
...
function assertParentDirectory(filePath) {
  ...
  try {
    ...
    fail('builder_agent_delegation_result_store_unavailable');
  }
...
function userVersion(db) {
  ...
  if (!row || !Number.isSafeInteger(row.user_version)) {
    fail('builder_agent_delegation_result_store_integrity_failed');
  }
...
function configurePragmas(db) {
  ...
  if (mode !== 'wal') fail('builder_agent_delegation_result_store_unavailable');
...
function createSchema(db) {
  ...
  try {
    ...
    db.exec(`PRAGMA user_version = ${BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION}`);
    db.exec('COMMIT');
  } catch (error) {
...
function expectedFingerprint() {
  ...
  try {
    ...
    expectedDb.exec(`PRAGMA user_version = ${BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION}`);
...
function validateSchema(db) {
  ...
  if (actual.foreign_key_check.length !== 0) fail('builder_agent_delegation_result_store_integrity_failed');
...
function initialize(db) {
  ...
  else if (version !== BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION) {
    fail('builder_agent_delegation_result_store_integrity_failed');
  }
  validateSchema(db);
}
...
function parseCanonicalReceipt(value, code) {
  ...
  try {
    ...
    if (error instanceof BuilderAgentDelegationResultStoreError) fail(code);
    throw error;
  }
  return parsed;
}
...
function sanitizeResultRequest(value) {
  ...
  try {
    return freezeDeep({
      delegation,
      result: sanitizeBuilderAgentDelegationResultRecord(result, delegation),
    });
  } catch (error) {
...
function resultColumns() {
  return `delegation_result_id, delegation_id, delegation_definition_digest,
    ...
    observed_at_ms, result_status, summary_code, return_contract,
...
function entryFromRow(row) {
  ...
  try {
    const delegation = parseCanonicalReceipt(
      row.delegation_json,
      'builder_agent_delegation_result_store_integrity_failed',
    );
    const parsedResult = parseCanonicalReceipt(
      row.result_json,
      'builder_agent_delegation_result_store_integrity_failed',
    );
    const result = sanitizeBuilderAgentDelegationResultRecord(parsedResult, delegation);
    const rowFacts = freezeDeep({
      ...
      return_contract: row.return_contract,
      parent_materialization_boundary: row.parent_materialization_boundary,
    });
    if (
      rowFacts.delegation_result_id !== result.delegation_result_id
      || rowFacts.delegation_id !== result.delegation_id
      || rowFacts.delegation_definition_digest !== result.delegation_definition_digest
      || rowFacts.target_definition_digest !== result.target_definition_digest
      || rowFacts.record_version !== BUILDER_AGENT_DELEGATION_RESULT_RECORD_VERSION
      || rowFacts.record_version !== result.record_version
      || rowFacts.parent_assignment_id !== result.parent_assignment_id
      || rowFacts.parent_assignment_status_id !== result.parent_assignment_status_id
      || rowFacts.parent_lease_id !== result.parent_lease_id
      || rowFacts.from_agent_id !== result.from_agent_id
      || rowFacts.from_agent_version_id !== result.from_agent_version_id
      || rowFacts.to_agent_id !== result.to_agent_id
      || rowFacts.to_agent_version_id !== result.to_agent_version_id
      || rowFacts.owner_id !== result.owner_id
      || rowFacts.project_id !== result.project_id
      || rowFacts.parent_conversation_id !== result.parent_conversation_id
      || rowFacts.parent_task_id !== result.parent_task_id
      || rowFacts.parent_run_id !== result.parent_run_id
      || rowFacts.child_conversation_id !== result.child_conversation_id
      || rowFacts.child_task_id !== result.child_task_id
      || rowFacts.child_run_id !== result.child_run_id
      ...
      || rowFacts.return_contract !== result.return_contract
      || rowFacts.parent_materialization_boundary !== result.parent_materialization_boundary
    ) fail('builder_agent_delegation_result_store_integrity_failed');
    return freezeDeep({ delegation, result });
  } catch (error) {
    if (
      error instanceof BuilderAgentDelegationResultStoreError
      || error instanceof BuilderAgentDelegationResultContractError
    ) fail('builder_agent_delegation_result_store_integrity_failed');
    throw error;
  }
}
...
function loadEntryByResultId(db, delegationResultId) {
  return entryFromRow(one(
    db,
    `SELECT ${resultColumns()} FROM agent_delegation_results WHERE delegation_result_id = ?`,
    [delegationResultId],
  ));
}
...
function loadEntryByDelegationId(db, delegationId) {
  return entryFromRow(one(
    db,
    `SELECT ${resultColumns()} FROM agent_delegation_results WHERE delegation_id = ?`,
    [delegationId],
  ));
}
...
function parentTaskEntries(db, ownerId, projectId, parentTaskId) {
  const rows = all(
    ...
    `SELECT ${resultColumns()}
      FROM agent_delegation_results
      WHERE owner_id = ? AND project_id = ? AND parent_task_id = ?
      ORDER BY observed_at_ms ASC, delegation_result_id ASC
      LIMIT ?`,
    [ownerId, projectId, parentTaskId, MAX_TASK_RESULTS + 1],
  );
  if (rows.length > MAX_TASK_RESULTS) fail('builder_agent_delegation_result_store_resource_exceeded');
...
function childTaskEntries(db, ownerId, projectId, childTaskId) {
  const rows = all(
    ...
    `SELECT ${resultColumns()}
      ...
      ORDER BY observed_at_ms ASC, delegation_result_id ASC
      LIMIT ?`,
    [ownerId, projectId, childTaskId, MAX_TASK_RESULTS + 1],
  );
  if (rows.length > MAX_TASK_RESULTS) fail('builder_agent_delegation_result_store_resource_exceeded');
  return freezeDeep(rows.map(entryFromRow));
}
...
function evidence(db, transaction) {
  return freezeDeep({
    ...
    schema_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_SCHEMA_VERSION,
    user_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION,
    schema_fingerprint_digest: sha256Canonical(collectSchemaFingerprint(db)),
    runtime_pragmas: runtimePragmas(db),
    transaction,
    delegation_result_authority: 'main_owned_agent_delegation_result_store',
...
function storeResult(db, operation, payload) {
  return freezeDeep({
    result_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_RESULT_VERSION,
    operation,
    ...payload,
    delegation_result_evidence: evidence(db, operation),
  });
}

function insertResult(db, entry) {
  const result = entry.result;
  run(db, `INSERT INTO agent_delegation_results (
    delegation_result_id, delegation_id, delegation_definition_digest,
    target_definition_digest, record_version, parent_assignment_id,
    parent_assignment_status_id, parent_lease_id, from_agent_id,
    from_agent_version_id, to_agent_id, to_agent_version_id, owner_id,
    project_id, parent_conversation_id, parent_task_id, parent_run_id,
    child_conversation_id, child_task_id, child_run_id, lease_holder_id,
    observed_at_ms, result_status, summary_code, return_contract,
    parent_materialization_boundary, delegation_json, result_json, schema_version
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
    result.delegation_result_id,
    result.delegation_id,
    result.delegation_definition_digest,
    result.target_definition_digest,
    result.record_version,
    result.parent_assignment_id,
    result.parent_assignment_status_id,
    result.parent_lease_id,
    result.from_agent_id,
    result.from_agent_version_id,
    result.to_agent_id,
    result.to_agent_version_id,
    result.owner_id,
    result.project_id,
    ...
    result.return_contract,
    result.parent_materialization_boundary,
    canonicalJson(entry.delegation),
    canonicalJson(result),
    BUILDER_AGENT_DELEGATION_RESULT_STORE_SCHEMA_VERSION,
  ]);
}

function recordDelegationResult(db, rawRequest) {
  const requested = sanitizeResultRequest(rawRequest);
  db.exec('BEGIN IMMEDIATE');
  try {
    const existing = loadEntryByResultId(db, requested.result.delegation_result_id);
    if (existing !== null) {
      if (!sameEntry(existing, requested)) fail('builder_agent_delegation_result_store_conflict');
      db.exec('COMMIT');
      return storeResult(db, 'delegation_result_replayed', { delegation_result: existing });
    }
    if (loadEntryByDelegationId(db, requested.result.delegation_id) !== null) {
      fail('builder_agent_delegation_result_store_conflict');
    }
    insertResult(db, requested);
    const readback = loadEntryByResultId(db, requested.result.delegation_result_id);
    if (readback === null || !sameEntry(readback, requested)) {
      fail('builder_agent_delegation_result_store_integrity_failed');
    }
    db.exec('COMMIT');
    return storeResult(db, 'delegation_result_recorded', { delegation_result: readback });
  } catch (error) {
    try { db.exec('ROLLBACK'); } catch { /* fixed failure below */ }
    throw error;
  }
...
function readDelegationResult(db, rawRequest) {
  ...
  const entry = loadEntryByResultId(db, delegationResultId);
  if (entry === null || entry.result.owner_id !== ownerId) {
    return freezeDeep({
      result_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION,
      delegation_result_authority: 'main_owned_agent_delegation_result_store',
      status: 'absent',
      delegation_result_id: delegationResultId,
      owner_id: ownerId,
      delegation_result: null,
      evidence: evidence(db, 'delegation_result_absent_read'),
    });
  }
  return freezeDeep({
    result_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION,
    ...
    delegation_result_id: delegationResultId,
    owner_id: ownerId,
    delegation_result: entry,
    evidence: evidence(db, 'delegation_result_ready_read'),
  });
}

function listParentTaskDelegationResults(db, rawRequest) {
  exactObject(rawRequest, LIST_PARENT_TASK_RESULTS_KEYS);
  const ownerId = safeOwnerId(valueAt(rawRequest, 'owner_id'));
  ...
  const results = parentTaskEntries(db, ownerId, projectId, parentTaskId);
  return freezeDeep({
    result_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION,
    delegation_result_authority: 'main_owned_agent_delegation_result_store',
    ...
    evidence: evidence(db, results.length === 0 ? 'parent_task_delegation_results_absent_read' : 'parent_task_delegation_results_ready_read'),
  });
}

function listChildTaskDelegationResults(db, rawRequest) {
  exactObject(rawRequest, LIST_CHILD_TASK_RESULTS_KEYS);
  const ownerId = safeOwnerId(valueAt(rawRequest, 'owner_id'));
  const projectId = safeProjectId(valueAt(rawRequest, 'project_id'));
  const childTaskId = safeTaskId(valueAt(rawRequest, 'child_task_id'));
  const results = childTaskEntries(db, ownerId, projectId, childTaskId);
  return freezeDeep({
    result_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION,
    delegation_result_authority: 'main_owned_agent_delegation_result_store',
    status: results.length === 0 ? 'absent' : 'ready',
    ...
    delegation_results: results,
    evidence: evidence(db, results.length === 0 ? 'child_task_delegation_results_absent_read' : 'child_task_delegation_results_ready_read'),
  });
}
...
function normalizeOperationError(error) {
  if (error instanceof BuilderAgentDelegationResultStoreError) {
    return new BuilderAgentDelegationResultStoreError(error.code);
  }
  if (error instanceof BuilderAgentDelegationResultContractError) {
    return new BuilderAgentDelegationResultStoreError('builder_agent_delegation_result_store_invalid');
  }
  ...
  if (sqliteCode && /^SQLITE_CONSTRAINT/u.test(sqliteCode)) {
    return new BuilderAgentDelegationResultStoreError('builder_agent_delegation_result_store_integrity_failed');
  }
  ...
  return new BuilderAgentDelegationResultStoreError('builder_agent_delegation_result_store_unavailable');
}

function createBuilderAgentDelegationResultStore(databasePath) {
  ...
  return freezeDeep({
    store_version: BUILDER_AGENT_DELEGATION_RESULT_STORE_VERSION,

    close(...rawArguments) {
      if (rawArguments.length !== 0) {
        throw new BuilderAgentDelegationResultStoreError('builder_agent_delegation_result_store_invalid');
      }
      try { db.close(); } catch (error) { throw normalizeOperationError(error); }
    },

    record_result(rawRequest) {
      try { return recordDelegationResult(db, rawRequest); } catch (error) {
        throw normalizeOperationError(error);
      }
    },

    read_result(rawRequest) {
      try { return readDelegationResult(db, rawRequest); } catch (error) {
        throw normalizeOperationError(error);
      }
    },

    list_parent_task_results(rawRequest) {
      try { return listParentTaskDelegationResults(db, rawRequest); } catch (error) {
        throw normalizeOperationError(error);
      }
    },

    list_child_task_results(rawRequest) {
      try { return listChildTaskDelegationResults(db, rawRequest); } catch (error) {
        throw normalizeOperationError(error);
      }
    },
  });
...
module.exports = Object.freeze({
  BUILDER_AGENT_DELEGATION_RESULT_STORE_READ_RESULT_VERSION,
  BUILDER_AGENT_DELEGATION_RESULT_STORE_RESULT_VERSION,
  BUILDER_AGENT_DELEGATION_RESULT_STORE_SCHEMA_VERSION,
  BUILDER_AGENT_DELEGATION_RESULT_STORE_USER_VERSION,
