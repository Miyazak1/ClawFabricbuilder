...
a digest-bound admission record. The Conversation main service can now append
that already-created admission through a trusted-context-only
`record_agent_step_progress` method, and Conversation replay plus the
renderer-safe Task Stream can expose it as public
